...
func (analyzer *Analyzer) process(msg *workers.Msg) {
	...
	if err != nil {
		lgr.Printf("unable to determine message type - %s\n", err)
		return
	}

	// fmt.Println(submissionKey, submissionType)
	...
	case "hello_world":
		fmt.Println("submitting hello world comment")

		comments = append(comments, []byte("Hello World Comment"))
